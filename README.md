...
각 후보는 추천 분량, 분량 근거, 단계별 전개를 함께 제안한다. critic이 선택한
후보가 3단계 이상이면 별도 승인 없이 진행하고, 1단계 또는 2단계이면 사용자
승인을 기다린다. 선택된 제목, 장르, 승인 분량은 이후 세계관 생성에서 변경할
수 없는 계약으로 검증된다.

분량을 직접 지정하면 Forge가 선택 시놉시스를 해당 단계 수에 맞춰 재설계한 뒤
즉시 확정한다.
...
게임 시나리오 모드는 선택한 기획으로 세계관 원고와 시나리오 패키지를 만든 뒤
종료한다. 장편 구조, 산문, EPUB 단계는 실행하지 않는다. 산출물은
`exports/game-scenario.json`과 `exports/game-scenario.md`다. 후보 목록, critic 평가,
선택 결과는 각각
...
critic 추천, 후보별 핵심 플레이 반복, 성장, 선택 구조, 추천 분량과 단계별
전개를 확인한다. 3단계 이상 추천은 자동으로 시나리오 패키지 생성에 진입하고,
1~2단계 추천만 승인 화면에서 멈춘다. 분량 입력란에 값을 넣으면 그 단계 수로
시놉시스를 재설계한다. 후보 생성은 현재 작품을 건드리지 않으므로 마음에 들
때까지 새 후보 5개를 반복 생성할 수 있다. 시나리오 생성은 후보를 선택하고
시작 버튼을 누른 뒤에만 진행된다. 작업은 서버의 백그라운드에서 진행되며
화면이 잠겨도 계속된다.
...
`--new-world` 장편 자동 실행 순서는 구조 후보 생성, 원천 정본 critic 검증,
정본 승격, 상태 원장 재구성, 장편 규모 확장, 재검증·재승격, 현재 권 산문
생성·critic 승인, 현재 권 검증과 EPUB 생성 순서다. 마지막 권이 끝난 실행에서는
전체 구조와 산문을 다시 검증하고 전체 EPUB을 확인한다. 게임 시나리오 모드는
세계관 원고와 패키지 생성 뒤 `scenario_complete` 상태로 끝난다. 진행 상태는
