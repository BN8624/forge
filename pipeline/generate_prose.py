...
def dialogue_required(context: dict[str, Any]) -> bool:
    return context["scene"]["dialogue_policy"] == "required"


def build_dialogue_instruction(context: dict[str, Any]) -> str:
    scene = context["scene"]
    policy = scene["dialogue_policy"]
    mode = scene["interaction_mode"]
    if policy == "required":
        return (
            f"- 이 장면의 상호작용 유형은 {mode}, 대화 정책은 required다. "
            "산문 분량의 약 15-35%를 최소 4개 발화의 대화로 구성한다. "
            "대화는 요구, 거절, 의심, 협상, 갈등, 관계 변화 또는 행동 결정을 만든다."
        )
    if policy == "optional":
        return (
            f"- 이 장면의 상호작용 유형은 {mode}, 대화 정책은 optional이다. "
            "상황에 자연스러울 때만 짧은 대화를 사용하며 분량을 맞추려고 강제하지 않는다."
        )
    return (
        f"- 이 장면의 상호작용 유형은 {mode}, 대화 정책은 none이다. "
        "대화량을 채우려 하지 말고 행동, 환경 반응, 선택과 결과로 장면을 진행한다."
    )
...
{build_dialogue_instruction(context)}
...
def build_critic_prompt(
    ...
    canon, _ = load_source_material()
    dialogue_rule = (
        "dialogue_policy가 required이므로 대화가 장면의 행동·관계·계획을 "
        "실제로 바꾸지 않거나 대화가 거의 없으면 prose_quality를 false로 판정한다."
        if dialogue_required(context)
        else "dialogue_policy가 required가 아니므로 대화량 부족만으로 실패시키지 않는다."
    )
...
- {dialogue_rule}
- 사용된 대화가 정보 전달용 독백이나 일방적인 설정 강의임.
