# 기존 정본 자료에서 승인 권수에 맞는 검증 가능한 구조 문서 후보를 생성하는 도구
...
DOCUMENT_IDS = {
    "volumes": re.compile(r"^V(?:[1-9]|[1-9][0-9])$"),
    "events": re.compile(r"^V(?:[1-9]|[1-9][0-9])-E\d{2}$"),
    "scenes": re.compile(r"^V(?:[1-9]|[1-9][0-9])-E\d{2}-S\d{2}$"),
}
...
def approved_volume_count() -> int:
    path = CURRENT_REFERENCE_ROOT / "selected-synopsis.json"
    try:
        selected = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return 5
    count = selected.get("approved_volume_count")
    return count if type(count) is int and 1 <= count <= 99 else 5
...
def build_prompt(instruction: str = "") -> str:
    ...
    volume_count = approved_volume_count()
    return f"""아래 기존 작품을 정확히 {volume_count}권인 장편소설 구조로 재설계하라.
...
기존 권수 설계는 입력이 아니며 {volume_count}권별 사건 배치는 스스로 새로 결정한다.
series.volume_ids는 V1부터 V{volume_count}까지 정확히 순서대로 사용한다.
    ...
    try:
        ...
        for attempt in range(1, MAX_GENERATION_ATTEMPTS + 1):
            ...
            try:
                ...
                expected_count = approved_volume_count()
                if bundle["series"].get("volume_ids") != [
                    f"V{index}"
                    for index in range(1, expected_count + 1)
                ]:
                    last_errors.append(
                        f"승인 권수 불일치: 정확히 {expected_count}권이어야 함"
                    )
                if last_errors:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="기존 정본 자료에서 승인 권수의 구조 문서 후보를 생성한다."
    )
    parser.add_argument(
        "--instruction-file",
        type=Path,
