...
Forge 독립 프로젝트 생성, 구조 검증기, 후보 정본 승격 파이프라인 구현이 끝났다. 아직 모델 호출이나 신규 원고 생성은 시작하지 않았다.
...
1. 승격 결과에서 상태 원장을 재구성하고 재실행 결과가 같은지 검사한다.
2. 구조 문서 후보 생성기를 구현한다.
3. 5권 전체의 신규 시리즈 초안을 구조 문서로 생성한다.
4. 권, 사건, 장면 순서로 세분화한 뒤 산문 생성을 시작한다.
